...
<nav class="navbar navbar-expand-lg navbar-dark ftco_navbar bg-dark ftco-navbar-light" id="ftco-navbar">
  ...
  <section class="ftco-appointment ftco-section bg-light">
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" name="fullName" id="fullName" required placeholder="Enter your full name">
    </div>
    <div class="form-group">
        <label for="mobile">Mobile Number</label>
        <input type="tel" class="form-control" name="mobile" id="mobile" required placeholder="Enter your mobile number">
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        <label for="aadhar">Aadhar (Upload file)</label>
...
$message = ""; // Initialize an empty message to be displayed after form submission
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['aadhar']) && $_FILES['aadhar']['error'] === UPLOAD_ERR_OK) {
        ...
        $targetFile = $targetDir . basename($aadharFile["name"]);

        // Validating the file (file type, size, etc.)
        $fileType = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));
        $allowedFormats = ["jpg", "jpeg", "png", "pdf"];
        $uploadOk = 1;

        if ($aadharFile["size"] > 2000000) {
            $message = "File is too large (2MB max).";
            $uploadOk = 0;
        }
        ...
        if ($uploadOk == 1) {
            if (move_uploaded_file($aadharFile["tmp_name"], $targetFile)) {
                // Insert the record into the database
                $stmt = $conn->prepare("INSERT INTO adoption_requests (full_name, mobile_number, address, aadhar_file, pet_type) VALUES (?, ?, ?, ?, ?)");
                $stmt->bind_param("sssss", $fullName, $mobile, $address, $targetFile, $petType);
